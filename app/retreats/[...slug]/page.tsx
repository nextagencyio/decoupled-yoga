...
import { GET_RETREAT_BY_PATH } from '@/lib/queries'
...
interface RetreatByPathData {
  route: {
    entity: DrupalRetreat
  } | null
}

async function getRetreat(path: string): Promise<DrupalRetreat | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<RetreatByPathData>({
      query: GET_RETREAT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/retreats/${slug.join('/')}`
  const item = await getRetreat(path)

  if (!item) {
    return { title: 'Retreat Not Found | Serenity Yoga Studio' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).spotsAvailable && (
                    <div>
                      <dt className="text-sm text-gray-500">Spots Available</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).spotsAvailable}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
